  ...
  app.command('/accessmate', async ({ command, ack, respond, client, context, logger }) => {
    ...
    try {
      ...
      switch (subcommand) {
        ...
        case 'alt':
          await handleAltText({ command, respond, client, context, logger });
          break;
        ...
        case 'digest': {
          ...
          try {
            ...
            logger?.error?.('Digest command failed:', err);
            await respond({
              text: '❌ Digest failed. Try again or contact the admin.',
              response_type: 'ephemeral',
              replace_original: false,
            });
          }
      ...
      try {
        await respond({
          text: '⚠️ Something went wrong. Try again or contact the admin.',
          response_type: 'ephemeral',
        });
      } catch (notifyErr) {
        logger?.error?.('Failed to notify user of command error:', notifyErr);
      }
    }
  });
}
