import { AccessMateDeps, runAccessMate } from '../../agent/index.js';
...
export async function handleAppMentioned({ client, context, event, logger, say, sayStream, setStatus }) {
  try {
    ...
    // Run the agent
    const deps = new AccessMateDeps(client, userId, channelId, threadTs, event.ts, context.userToken);
    const result = await runAccessMate(inputItems, deps);
