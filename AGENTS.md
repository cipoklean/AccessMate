...
- **AccessMateDeps** — verify constructor stores all properties
...
`agent/deps.js` defines a `AccessMateDeps` class that holds `client`, `userId`, `channelId`, `threadTs`, `messageTs`, and `userToken`. This is passed as the `context` parameter to `run()` and is available in tool `execute` functions via `context.context`.

### Slack MCP Server

`support-agent.js` exports a `runAccessMate(inputItems, deps)` function that wraps agent execution. When `deps.userToken` is present, it connects to the Slack MCP Server (`https://mcp.slack.com/mcp`) via `MCPServerStreamableHttp`, clones the agent with the MCP server attached, and runs it. The MCP connection is always closed in a `finally` block.
