...
/** Hostnames we consider safe for Slack file downloads. */
const ALLOWED_HOSTNAMES = ['files.slack.com', 'files-private.slack.com'];
...
/**
 * Single download attempt via node:https with TLS 1.2+ and curl UA.
 * Follows up to 2 redirects — only to allowed Slack hostnames.
 * 15s timeout.
 */
function downloadOnce(urlStr, slackToken, redirectsLeft = 2) {
  return new Promise((resolve, reject) => {
    ...
    // SSRF protection: reject non-Slack hostnames
    if (!isAllowedHostname(url.hostname)) {
      return reject(new Error(`Blocked download from disallowed host: ${url.hostname}`));
    }
...
export async function downloadSlackFileAsDataUrl(urlStr, slackToken) {
  ...
  if (!token) throw new Error('No Slack token available for file download');
